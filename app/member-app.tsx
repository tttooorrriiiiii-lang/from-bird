...
function ProjectsPage(){const [items,setItems]=useState<CmsContent[]>(fallbackProjects);useEffect(()=>{let alive=true;supabase.from("contents").select("id,title,slug,description,url,thumbnail_path,is_featured,sort_order").eq("status","published").order("sort_order",{ascending:true}).then(({data,error})=>{if(alive&&!error&&data?.length)setItems(data as CmsContent[])});return()=>{alive=false}},[]);return <div className="projects-index"><header><div><span>PROJECT INDEX / {String(items.length).padStart(2,"0")}</span><h1>どこから、<br/>考える？</h1></div><p>完成品を見る場所ではなく、<br/>見方を動かすための道具棚。</p><i aria-hidden="true">↘</i></header><nav aria-label="プロジェクトの分類"><span>ALL</span><span>WORDS</span><span>COLOR</span><span>SHAPE</span><span>STORY</span><span>THINK</span></nav><section className="projects-shelf">{items.map((p,i)=><article key={p.id}><b>{String(i+1).padStart(2,"0")}</b><ProjectCard project={p}/></article>)}</section></div>}
function NewsPage(){const [items,setItems]=useState<CmsNews[]>(fallbackNews);useEffect(()=>{let alive=true;supabase.from("news").select("id,title,body,link_url,published_at").eq("status","published").order("published_at",{ascending:false}).then(({data,error})=>{if(alive&&!error&&data?.length)setItems(data as CmsNews[])});return()=>{alive=false}},[]);return <div className="news-journal"><header><span>NEWS / JOURNAL</span><h1>鳥の足あと。</h1><p>つくったもの、変わったこと。<br/>FROM BIRDの時間を記録します。</p></header><section>{items.map((item,i)=><a href={item.link_url||"#"} target={item.link_url?"_blank":undefined} rel={item.link_url?"noreferrer":undefined} key={item.id}><div className="news-date"><b>{newsDate(item.published_at).slice(5)}</b><small>{newsDate(item.published_at).slice(0,4)}</small></div><i/><div className="news-entry"><span>NOTE {String(items.length-i).padStart(2,"0")}</span><h2>{item.title}</h2>{item.body&&<p>{item.body}</p>}</div><strong>↗</strong></a>)}</section></div>}
function AboutPage(){return <div className="about-manifesto"><section className="about-opening"><div><span>ABOUT FROM BIRD</span><h1>考えることを、<br/>遊びに変える。</h1><p>答えより先に、見方を増やす。</p></div><div className="about-bird" aria-hidden="true"><i/><i/><div className="paper-bird"><span className="bird-tail"/><span className="bird-body"/><span className="bird-head"/><span className="bird-wing bird-wing-top"/><span className="bird-wing bird-wing-bottom"/><span className="bird-beak"/><span className="bird-eye"/></div><em>ANOTHER<br/>VIEW</em></div></section><section className="about-flow"><p>ことば</p><i>→</i><p>色</p><i>→</i><p>かたち</p><i>→</i><p>問い</p><i>→</i><p>物語</p></section><section className="about-message"><span>WHY WE PLAY</span><h2>違う入口を歩くたび、<br/>自分の見方が残っていく。</h2><p>同じものを見ても、残る言葉や色、気になる問いは人によって違います。FROM BIRDは、正しさを測る診断ではありません。小さな実験を行き来しながら、自分でも知らなかった表現の癖を見つける場所です。</p></section><section className="about-principles"><article><b>THINK</b><span>01</span><p>答えを急がず、<br/>問いを眺める。</p></article><article><b>PLAY</b><span>02</span><p>うまく作るより、<br/>まず試す。</p></article><article><b>EXPLORE</b><span>03</span><p>痕跡から、<br/>自分の視点を探す。</p></article></section><a className="about-join" href="/signup" onClick={nav("/signup")}><span>自分の変化を残す</span><b>JOIN FROM BIRD　→</b></a></div>}
function ContactPage(){function send(e:FormEvent<HTMLFormElement>){e.preventDefault();const fd=new FormData(e.currentTarget);const subject=encodeURIComponent(`[FROM BIRD] ${String(fd.get("subject")||"お問い合わせ")}`);const body=encodeURIComponent(`お名前：${String(fd.get("name")||"")}\nメール：${String(fd.get("email")||"")}\n\n${String(fd.get("message")||"")}`);location.href=`mailto:hello@frombird.com?subject=${subject}&body=${body}`}return <div className="contact-letter"><aside><span>CONTACT</span><h1>まだ言葉に<br/>ならないことも。</h1><p>感想、質問、いっしょに考えてみたいこと。<br/>短いひとことから、どうぞ。</p><div className="contact-flight" aria-hidden="true"><i/><i/><b>◆</b></div><dl><dt>MAIL</dt><dd>hello@frombird.com</dd><dt>REPLY</dt><dd>通常3営業日以内</dd></dl></aside><form onSubmit={send}><header><span>WRITE A LETTER</span><b>FROM ________</b></header><label><span>01　お名前</span><input name="name" placeholder="お名前を入力" required/></label><label><span>02　メールアドレス</span><input name="email" type="email" placeholder="mail@example.com" required/></label><label><span>03　どんなこと？</span><select name="subject"><option>サービスについて</option><option>作品・ツールについて</option><option>コラボレーションについて</option><option>その他</option></select></label><label><span>04　メッセージ</span><textarea name="message" rows={7} placeholder="ここに書いてください。" required/></label><button>この手紙をひらく <span>→</span></button><small>ボタンを押すと、お使いのメールアプリが開きます。</small></form></div>}
