...
function PublicPage({route,user}:{route:string;user:User|null}){return <main className={`public-page ${route==="/about"?"public-about":""}`}><PublicHeader user={user}/>{route==="/projects"&&<ProjectsPage/>}{route==="/news"&&<NewsPage/>}{route==="/about"&&<AboutPage/>}{route==="/contact"&&<ContactPage/>}<PublicFooter/></main>}
function ProjectsPage(){const [items,setItems]=useState<CmsContent[]>(fallbackProjects);useEffect(()=>{let alive=true;supabase.from("contents").select("id,title,slug,description,url,thumbnail_path,is_featured,sort_order").eq("status","published").order("sort_order",{ascending:true}).then(({data,error})=>{if(alive&&!error&&data?.length)setItems(data as CmsContent[])});return()=>{alive=false}},[]);return <div className="projects-editorial"><svg className="flight-line" viewBox="0 0 1200 150" preserveAspectRatio="none" aria-hidden="true"><path d="M0 40 C210 5 350 30 500 85 S770 125 930 82 S1100 35 1200 70"/><path d="m468 73 16 12-18 5"/><path d="m945 72-15 12 17 6"/></svg><header><span>PROJECTS</span><h1>どこから、考える？</h1><p>ことば、色、かたち、そして問い。<br/>さまざまな視点から、<br/>社会や文化を見つめるプロジェクトです。</p><a href="#project-list">ALL PROJECTS　→</a></header><section id="project-list">{items.slice(0,5).map((p,i)=><a href={p.url||"#"} target={p.url?"_blank":undefined} rel={p.url?"noreferrer":undefined} className={`project-tile tile-${i+1}`} key={p.id}><div className="tile-art"><i/><i/></div><span>{String(i+1).padStart(3,"0")}</span><h2>{p.title}</h2><p>{p.description||"新しい見方を探す。"}</p><b>→</b></a>)}</section></div>}
function NewsPage(){const [items,setItems]=useState<CmsNews[]>(fallbackNews);useEffect(()=>{let alive=true;supabase.from("news").select("id,title,body,link_url,published_at").eq("status","published").order("published_at",{ascending:false}).then(({data,error})=>{if(alive&&!error&&data?.length)setItems(data as CmsNews[])});return()=>{alive=false}},[]);return <div className="news-flight"><header><span>NEWS</span><h1>鳥の足あと。</h1><p>活動のお知らせ、イベント、<br/>掲載情報など、FROM BIRDの、<br/>いまをお届けします。</p><a href="#news-list">ALL NEWS　→</a><small>足あとは、<br/>つぎの景色へ<br/>つながっている。</small></header><section id="news-list"><svg viewBox="0 0 100 700" preserveAspectRatio="none" aria-hidden="true"><path d="M54 0 C5 72 93 122 44 205 S94 350 38 445 S83 570 45 700"/></svg>{items.slice(0,5).map((item,i)=><a href={item.link_url||"#"} target={item.link_url?"_blank":undefined} rel={item.link_url?"noreferrer":undefined} key={item.id}><i className={`feather feather-${i+1}`}/><div><time>{newsDate(item.published_at)}</time><h2>{item.title}</h2></div><b>→</b></a>)}</section><aside><p>ことばは、<br/>どこかへ向かっていく。</p><i/><small>過去のお知らせも、<br/>どこかで、つながっている。</small></aside></div>}
function AboutPage(){return <div className="about-sky"><section className="about-copy"><span>ABOUT FROM BIRD</span><h1>考えることを、<br/>遊びに変える。</h1><p>FROM BIRDは、<br/>ことば・色・かたちを手がかりに、<br/>社会や文化を見つめ、問いを育てる、<br/>アート＆デザインの実験室です。</p><a href="/projects" onClick={nav("/projects")}>私たちの実験を見る　→</a><small>まだ見ぬ視点へ、<br/>ともに。</small></section><svg className="about-bird-svg" viewBox="0 0 640 680" role="img" aria-label="空を飛ぶ抽象的な鳥"><path className="svg-body" d="M320 335C365 165 470 70 600 45C578 200 485 315 320 335Z"/><path className="svg-wing-light" d="M490 115C530 78 568 55 600 45C595 115 575 180 538 228Z"/><path className="svg-tail-dark" d="M320 335C225 355 125 450 90 610C225 565 295 455 320 335Z"/><path className="svg-tail-light" d="M90 610C105 540 128 490 165 445C180 505 165 565 90 610Z"/><path className="svg-head" d="M320 335C275 280 255 205 175 190C120 179 78 210 65 258C135 300 230 325 320 335Z"/><path className="svg-beak" d="M66 218L5 258L66 297Z"/><circle className="svg-eye" cx="150" cy="225" r="18"/><path className="svg-orb-neck" d="M320 335C329 383 339 430 350 467L304 500C306 438 311 382 320 335Z"/><circle className="svg-orb" cx="350" cy="510" r="58"/></svg><p className="about-bird-note">飛ぶのは、<br/>考えるためだ。</p><div className="about-orbit" aria-label="FROM BIRDの5つの入口"><b>FROM<br/>BIRD</b><a href="https://kotoba.frombird.com/">ことば</a><span>色</span><span>かたち</span><span>問い</span><span>物語</span></div><svg className="about-horizon" viewBox="0 0 1200 70" preserveAspectRatio="none" aria-hidden="true"><path d="M0 34 C310 70 640 48 1200 22"/></svg></div>}
function ContactPage(){function send(e:FormEvent<HTMLFormElement>){e.preventDefault();const fd=new FormData(e.currentTarget);const subject=encodeURIComponent(`[FROM BIRD] ${String(fd.get("subject")||"お問い合わせ")}`);const body=encodeURIComponent(`お名前：${String(fd.get("name")||"")}\nメール：${String(fd.get("email")||"")}\n\n${String(fd.get("message")||"")}`);location.href=`mailto:hello@frombird.com?subject=${subject}&body=${body}`}return <div className="contact-night"><section><span>CONTACT</span><h1>まだ言葉に<br/>ならないことも。</h1><p>企画のご相談、取材のお申し込み、<br/>ご感想など、どんなことでもお気軽に<br/>お問い合わせください。</p><small>言葉を運ぶ、<br/>どこかのだれかへ。</small><svg viewBox="0 0 620 210" preserveAspectRatio="none" aria-hidden="true"><path d="M0 150 C145 82 210 210 320 145 S445 95 620 50"/><path d="m560 31 60 19-45 42z"/></svg></section><form onSubmit={send}><i className="fold"/><label>お名前<input name="name" required/></label><label>メールアドレス<input name="email" type="email" required/></label><label>お問い合わせの種類<select name="subject"><option>選択してください</option><option>サービスについて</option><option>作品・ツールについて</option><option>コラボレーションについて</option><option>その他</option></select></label><label>メッセージ<textarea name="message" rows={6} required/></label><button>送信する　→</button></form><aside>ことばは、<br/>どこかへ届く。<b>FROM<br/>BIRD</b><small>WORDS<br/>COLORS<br/>FORMS<br/>QUESTIONS<br/>STORIES</small></aside></div>}
